...
The Review Queue's QA Hold bucket is **view-only** (`isQaHoldView` is part of `isReadOnlyView` in `ReviewQueue.tsx`): it lists who is on hold with the matched plan/group/ID reasons but has no Verified in network / Confirm OON / Approve / OON / Decline buttons. All resolution happens in QA Operations.
