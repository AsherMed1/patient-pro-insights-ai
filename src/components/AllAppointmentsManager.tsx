  ...
  const fetchAppointments = async () => {
    try {
      ...
      if (activeTab === 'all') {
        ...
        // New: IPC NOT true AND not Pending (unless it's an unscheduled-capture lead, e.g. ECCO/Premier/Davis)
        countQuery = countQuery
      ...
      if (activeTab === 'all') {
        ...
        // New: IPC NOT true AND not Pending (unless unscheduled-capture lead)
        appointmentsQuery = appointmentsQuery
          .or('internal_process_complete.is.null,internal_process_complete.eq.false')
          .or('status.not.ilike.pending,is_unscheduled.eq.true')
          .not('status', 'ilike', 'do not call')
  ...
  const updateAppointmentStatus = async (appointmentId: string, status: string) => {
    try {
      ...
      const result = await changeAppointmentStatus({
        ...
        newStatus: status,
        userName,
        currentAppointment: currentAppointment as any,
      ...
      <Card className="w-full">
             ...
             <Button
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  const dateColumn = dateFilterType === 'created' ? 'date_appointment_created' : 'date_of_appointment';
                  let query = supabase.from('all_appointments').select('*').eq('review_status', 'approved').or('is_reserved_block.is.null,is_reserved_block.eq.false').or('is_superseded.is.null,is_superseded.eq.false');
        ...
        <CardContent className="p-3 md:p-6 pt-0">
          ...
          <AppointmentsTabs
            ...
            onDataRefresh={() => { fetchAppointments(); fetchTabCounts(); }}
            tabCounts={tabCounts}
