...
import { transformMultiSheetAppointmentData } from '@/utils/appointmentDataTransformer';
...
const CampaignDashboard = ({ clientId }: CampaignDashboardProps) => {
  ...
  // Try appointment-based transformation first (for Texas Vascular Institute)
  const appointmentData = hasDateRange && clientId === 'texas-vascular-institute' 
    ? transformMultiSheetAppointmentData(allData, 'texas vascular', dateRange) 
    : null;

  // Fallback to campaign-based transformation
  const campaignData = hasDateRange && !appointmentData 
    ? transformMultiSheetCampaignData(allData, procedure, dateRange) 
    : null;
  
  console.log('Campaign Dashboard - Appointment Data:', appointmentData);
  console.log('Campaign Dashboard - Campaign Data:', campaignData);
  console.log('Campaign Dashboard - Using Live Data:', !!(appointmentData || campaignData));
  
  // Convert appointment data to campaign data format if needed
  const data = appointmentData ? {
    adSpend: 0, // Not available in appointment data
    leads: 0, // Not available in appointment data
    appointments: appointmentData.totalAppointments,
    procedures: appointmentData.proceduresOrdered,
    showRate: appointmentData.showRate,
    cpl: 0, // Not available in appointment data
    cpa: 0, // Not available in appointment data
    cpp: 0, // Not available in appointment data
    trend: 'up' as const
  } : campaignData || {
    adSpend: 0,
    leads: 0,
  ...
  const isLiveData = !!(appointmentData || campaignData) && hasDateRange;
  const dataType = appointmentData ? 'appointments' : campaignData ? 'campaigns' : 'none';
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <Card className="bg-gradient-to-br from-blue-500 to-blue-600 text-white">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium opacity-90">
              {dataType === 'appointments' ? 'Total Appointments' : 'Total Ad Spend'}
            </CardTitle>
            {dataType === 'appointments' ? <Calendar className="h-4 w-4 opacity-90" /> : <DollarSign className="h-4 w-4 opacity-90" />}
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">
              {dataType === 'appointments' ? data.appointments : formatCurrency(data.adSpend)}
            </div>
        ...
        <Card className="bg-gradient-to-br from-green-500 to-green-600 text-white">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium opacity-90">
              {dataType === 'appointments' ? 'Procedures Ordered' : 'Leads Generated'}
            </CardTitle>
            <Users className="h-4 w-4 opacity-90" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">
              {dataType === 'appointments' ? data.procedures : data.leads}
            </div>
        ...
        <Card className="bg-gradient-to-br from-purple-500 to-purple-600 text-white">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium opacity-90">
              {dataType === 'appointments' ? 'Showed Appointments' : 'Appointments'}
            </CardTitle>
            <Calendar className="h-4 w-4 opacity-90" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">
              {dataType === 'appointments' ? (appointmentData?.showedAppointments || 0) : data.appointments}
            </div>
            <p className="text-xs opacity-90 mt-1">
              {dataType === 'appointments' ? 'Patients who showed' : 'Booked this period'}
            </p>
          </CardContent>
        </Card>
      ...
      {/* Cost Metrics - Show only for campaign data */}
      {dataType === 'campaigns' && (
            ...
            {dataType === 'appointments' ? (
              <>
                <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                  ...
                  <Badge variant={data.appointments > 0 && data.procedures / data.appointments > 0.3 ? "default" : "secondary"}>
                    {data.appointments > 0 && data.procedures / data.appointments > 0.3 ? "Good" : data.appointments > 0 ? "Needs Improvement" : "No Data"}
                  </Badge>
                </div>
                
                <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                  <div>
                    <h4 className="font-medium">Show Rate Performance</h4>
                    <p className="text-sm text-gray-600">
                      {formatPercent(data.showRate)} of scheduled appointments showed up
                    </p>
                  </div>
                  <Badge variant={data.showRate > 75 ? "default" : data.showRate > 0 ? "secondary" : "outline"}>
                    {data.showRate > 75 ? "Excellent" : data.showRate > 50 ? "Good" : data.showRate > 0 ? "Needs Improvement" : "No Data"}
                  </Badge>
                </div>
              </>
            ) : (
