...
// 10 diverse, realistic patient profiles for demo purposes
...
// Generate realistic insurance card URL
const generateInsuranceCardUrl = (provider: string, memberId: string) => {
  // Using a placeholder service for demo insurance cards
...
// Generate comprehensive patient intake notes
...
export const populatePPMTestAccountData = async () => {
  try {
    console.log('Starting PPM Test Account data population...');
    ...
    if (!appointments || appointments.length === 0) {
      console.log('No appointments found for PPM - Test Account');
    ...
    const updates = appointments.map(async (appointment, index) => {
    ...
    if (failureCount > 0) {
      console.log(`❌ Failed to update ${failureCount} appointments`);
    }
    ...
    console.error('Error in populatePPMTestAccountData:', error);
