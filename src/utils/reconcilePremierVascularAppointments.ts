...
// Expected CSV appointments from Premier_Vascular_Surgery_Tracking_Sheet_GAE_-_GAE.csv
...
function parseCSVDate(csvDate: string): { date: string; time: string } {
...
export async function reconcilePremierVascularAppointments() {
  console.log("🔍 Starting reconciliation of Premier Vascular appointments...");
  console.log(`📊 CSV has ${CSV_APPOINTMENTS.length} appointments`);
  
  // Get all current appointments from database
  const { data: dbAppointments, error } = await supabase
    .from('all_appointments')
    .select('lead_name, date_of_appointment, requested_time')
  ...
  console.log(`📊 Database has ${dbAppointments?.length} appointments`);
  
  // Create a map of database appointments for comparison
  // For people with multiple appointments, we track the latest one
  const dbMap = new Map<string, Set<string>>();
  dbAppointments?.forEach(appt => {
    const name = appt.lead_name.trim().toLowerCase();
    if (!dbMap.has(name)) {
      dbMap.set(name, new Set());
    }
    if (appt.date_of_appointment) {
      dbMap.get(name)!.add(appt.date_of_appointment);
    }
  });
  ...
  for (const csvAppt of CSV_APPOINTMENTS) {
    ...
    const { date } = parseCSVDate(csvAppt.apptDate);
    
    // Check if this specific appointment (name + date combo) exists
    if (!dbMap.has(normalizedName) || !dbMap.get(normalizedName)!.has(date)) {
  ...
  console.log(`\n🔍 Found ${missingAppointments.length} missing appointments:`);
  missingAppointments.forEach(appt => {
    console.log(`  - ${appt.fullName} (${appt.apptDate})`);
  });
  
  if (missingAppointments.length === 0) {
    console.log("✅ All CSV appointments are already in the database!");
    return { success: true, added: 0, missingAppointments: [] };
  }
  
  // Add missing appointments
  console.log(`\n📝 Adding ${missingAppointments.length} missing appointments...`);
  const appointmentsToInsert = missingAppointments.map(appt => {
    const { date, time } = parseCSVDate(appt.apptDate);
    return {
      project_name: 'Premier Vascular',
      lead_name: appt.fullName,
      date_appointment_created: date,
      date_of_appointment: date,
      requested_time: time,
      status: appt.status,
      calendar_name: appt.calendar,
      lead_phone_number: appt.phone,
      dob: appt.dob || null,
      internal_process_complete: false,
      was_ever_confirmed: appt.status.toLowerCase() === 'confirmed'
    };
  });
  ...
  if (insertError) {
    console.error("❌ Failed to insert appointments:", insertError);
    return { success: false, error: insertError, missingAppointments };
  }
  
  console.log(`✅ Successfully added ${inserted?.length} appointments`);
  ...
  console.log(`\n📊 Final count: ${finalCount} appointments`);
  console.log(`${finalCount === 155 ? '✅' : '⚠️'} Expected: 155 (${finalCount === 155 ? 'MATCH' : 'MISMATCH'})`);
...
if (import.meta.env.DEV) {
  console.log("💡 To run reconciliation: import { reconcilePremierVascularAppointments } from '@/utils/reconcilePremierVascularAppointments' and call it");
}
