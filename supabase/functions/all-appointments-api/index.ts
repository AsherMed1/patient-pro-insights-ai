...
serve(async (req) => {
  ...
  try {
    ...
    const UNSCHEDULED_PROJECTS = new Set(['premier vascular', 'ecco medical', 'davis vein & vascular', 'horizon vascular specialists', 'prospero vascular and interventional', 'vascular institute of michigan', 'alliance vascular']);
    ...
    if (data && data[0] && appointmentData.ghl_id && !appointmentData.insurance_id_link) {
      try {
        ...
        if (appointmentData.project_name) {
          ...
          if (project?.ghl_api_key) {
            console.log('Using project-specific GHL API key for:', appointmentData.project_name);
    ...
    if (record) {
      checkShortNoticeAlert(supabase, record);
    }
