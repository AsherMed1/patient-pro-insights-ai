...
-- Move any existing Reopened rows back into New, log activity
DO $$
DECLARE
  r RECORD;
BEGIN
  FOR r IN SELECT id, completed_at FROM public.qa_cases WHERE workflow_status = 'reopened'
  LOOP
    UPDATE public.qa_cases
      SET workflow_status = 'new',
          entered_queue_at = COALESCE(last_alert_activity_at, entered_queue_at, now()),
          completed_at = NULL,
          completed_by_user_id = NULL,
          date_resolved = NULL,
          resolution_type = NULL,
    ...
    INSERT INTO public.qa_case_activity (case_id, activity_type, description, metadata)
    VALUES (r.id, 'realerted', 'Migrated from Reopened → New',
            jsonb_build_object('previous_completed_at', r.completed_at));
  END LOOP;
END $$;
